...
- `defaultCompletionOptions`: Default completion options for model settings.
  ...
  - `thinkingOpenTag`: Custom opening tag for thinking/reasoning content extraction (e.g., `<think>`). Used with providers like vLLM that support custom thinking output formats.
  - `thinkingCloseTag`: Custom closing tag for thinking/reasoning content extraction (e.g., `</think>`). Must be used together with `thinkingOpenTag`.
